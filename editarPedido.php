...
<html lang="en">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <div id="divButtonImprimir" style="display : flex; flex-direction: row;  align-items : flex-end ; justify-content : flex-end; width:100%">
            <h1 class="h2">Editar Pedido con folio : <?php echo $_GET['idPedido'] ?></h1>
          </div>
          <div id="divButtonImprimir" style="display : flex; flex-direction: row;  align-items : flex-end ; justify-content : flex-end; width:100%">
        ...
        <div id="divAnadir" style="display : none">
          <div style="display : flex ; flex-direction : column" class="border-bottom" >
            <h3>Añadir articulo a pedido</h3>
            ...
            <div class="row g-3 align-items-center ">
                <div class="col mb-3">
                  ...
                  <div id="resultImg" class="row g-3 align-items-center">
            ...
            <div style="display : flex ; flex-direction : column" class="border-bottom" >
              ...
              <div class="row g-3 align-items-center ">
                  <div class="mb-3 col-md-4 d-flex flex-row" style= "margin-left:10px">
                    ...
                    <input type="date" id="fechaCreacionPedido" class="form-control">
                  </div>
                  <div class="mb-3 col-md-4 d-flex flex-row" style= "margin-left:10px">
                  ...
                  <div class="col mb-3">
                      <label for="nombreCliente" class="col-form-label">Folio Corte: </label>
                  </div>
                  <div class="col input-group mb-3">
  ...
  ?>
    ...
    async function cargarDatosInicialesPedido(){
      ...
      articulosPedido = json.data.articulos; 
      articulosPedido.map((art , index)=>{
        let imgsApi = art.imagenes ;
        art["imgsApi"] =imgsApi ; 
        art.imagenes = [] ;
      });
      document.getElementById("nombreCliente").value = json.data.nombreCliente;
      document.getElementById("numeroCliente").value = json.data.numeroCliente;
    ...
    window.onload = async function() {
      if (window.File && window.FileList && window.FileReader) {
        ...
        filesInput.addEventListener("change", function(event) {
          ...
          //console.log(files);
          let output = document.getElementById("resultImg");
    ...
    async function enterCodigoArticulo(event){   
      ...
      if(keycode == '13'){
          ...
          if(srcCodigo !== ""){
              ...
              document.getElementById("cantidad").focus() ;
              document.getElementById("totalText").value = "" ;

              //console.log(articuloSeleccionado.idArticulo , "  " ,artRequest.idArticulo);
              //articuloSeleccionado.idArticulo = artRequest.idArticulo ;
              //articuloSeleccionado.descripcionArticulo =artRequest.nombre ;
              //arit
          }else{
...
function agregarDatos(){
  ...
  if(indiceEditar == -1){// agregar articulo al pedido
    ...
    console.log(articuloSeleccionado);
    let articuloPedido = {
      idPedidoArticulo : null,
      idArticulo :articuloSeleccionado.idArticulo,
      claveArticulo :articuloSeleccionado.codigo,
    ...
    let articuloPedido = {} ;
    if(articuloSeleccionado == null){
      //console.log("No se selecciono ningun imagen por lo que se vuelve a venir lo mismo vd?")
      articuloPedido = {
        idPedidoArticulo : articuloEditar.idPedidoArticulo ,
        idArticulo : articuloEditar.idArticulo ,
        claveArticulo :articuloEditar.claveArticulo,
        nombreArticulo :articuloEditar.nombreArticulo,
        precioUnitario :articuloEditar.precioUnitario ,
        cantidad :  cantidadTxt , 
        total : (articuloEditar.precioUnitario * cantidadTxt),
        descripcionArticulo : descripcion,
        imagenes : auxImagenes,
        imgsApi :articuloEditar.imgsApi
      } ;
    }else{
      console.log("se selecciono un articulo en el enter")
      console.log(articuloSeleccionado) // ese articulo es el que se agarra cuando se le da un enter o viene de la modal 
      console.log(articuloEditar)// este es el que viene de la lista renderizada
      articuloPedido = {
        idPedidoArticulo : articuloEditar.idPedidoArticulo ,
        idArticulo :articuloSeleccionado.idArticulo,
        claveArticulo :articuloSeleccionado.codigo,
        ...
        precioUnitario :articuloSeleccionado.precio ,
        cantidad :  cantidadTxt , 
        total : (precioUnitario * cantidadTxt),
        descripcionArticulo : descripcion,
        imagenes : auxImagenes,
        imgsApi :articuloEditar.imgsApi
      } ;
    ...
    //console.log(articuloPedido);
    //console.log('Indice ' , indiceEditar)
    //console.log("Lista antes de editar");
    //console.log(articuloPedido)
    articulosPedido[indiceEditar] =articuloPedido ; 
    indiceEditar = -1 ;
    estaEditando = 0 ;
    //console.log("Lista despues de editar");
...
function renderTabla(){
    ...
    if(articulosPedido.length > 0){
        ...
        tableSalida += `<tr>
            ...
            <th></th>
            <th></th>
            <th><button class="btn btn-primary" onClick="guardarPedido()">Guardar Cambios</th>       
        </tr>`;
...
function editarArticulo(indice){
  //console.log(estaEditando)
    ...
    //totalText
    document.getElementById("totalText").value = articuloEditar.total ;
    let output = document.getElementById("resultImg");
    //--------let output = document.getElementById("resultImg");
    console.log(articuloEditar);
   ...
   // imagenes que vienen del api 
   console.log(articuloEditar);
   for(let i = 0 ; i < articuloEditar.imgsApi.length; i++){
    let imagen =articuloEditar.imgsApi[i];
    let div = document.createElement("div");
    div.classList.add("col");
    let salida = `<div class="card hoverable" style="width:400px; height:400px" id="apicde${i}">
                    <img class="card-img-top" <img src='${imagen.url}' style="width:400px; height:400px" alt="Card image" title="${imagen.nombreArchivo}">
                    <div class="card-img-overlay">
                      <div style="witdh:100%; display : flex; flex-direction : row; justify-content: end;">
                        <button class="btn btn-danger" onClick="removerImgApi(${i})"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" width="12" height="12"><path d="M2.22 2.22a.749.749 0 0 1 1.06 0L6 4.939 8.72 2.22a.749.749 0 1 1 1.06 1.06L7.061 6 9.78 8.72a.749.749 0 1 1-1.06 1.06L6 7.061 3.28 9.78a.749.749 0 1 1-1.06-1.06L4.939 6 2.22 3.28a.749.749 0 0 1 0-1.06Z"></path></svg></button>
                      </div>
                    </div>
                  </div>` ; 
    div.innerHTML = salida ;
...
function removerImgApi(index){
      
  document.getElementById('apicde' + index).remove();
  //dt.items.remove(index);
  articuloEditar.imgsApi =articuloEditar.imgsApi.filter((img, indx) => indx != index);
  console.log(articuloEditar.imgsApi);
...
// aqui ira la parte de peticion de los objetos 
  ...
  try{
    ...
    let fechaPedido = document.getElementById("fechaCreacionPedido").value ; 
    let fechaVencimiento = document.getElementById("fechaVencimiento").value ;
    idCorte = document.getElementById("folioCorte").value ;
    if (nombreCliente == "") {
    ...
    if(respuestaDialog.isConfirmed){
      let jsonRequest = {
          ...
          "fechaCreacionPedido" : fechaPedido, 
      ...
      let arrayFormData = [] ;
      // ids imagenes que seran borradas para actualizar en bd y borrarlas en el servidor 
      let ids = articulosPedido
                .filter(a => a.idPedidoArticulo !== null)
                .map(art => {
                  return {
                    idPedidoArticulo : art.idPedidoArticulo ,
                    idImagenes : art.imgsApi.map(img => img.idImagen).join(',')
                  }
                  
                });

      // ids de los pedidos que vienen desde el servidor 
      let idsPedidos = articulosPedido
                       .filter(a => a.idPedidoArticulo !== null)
                       .map( art => art.idPedidoArticulo).join(',');
      console.log("ids pedidos que vienen de la bd y no fueron borrados al editar")
      console.log(idsPedidos);
      let jsonArticulosBorrar = {
        "idPedido" : <?php echo $_GET['idPedido'] ?>,
        "idPedidosArticulos" : idsPedidos
      };
      const responseArticulosBorrar = await fetch('http://localhost/sieslite/api/pedidosImagenes.php?opc=3', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json'
          },
          body: JSON.stringify(jsonArticulosBorrar)
      });

      const resultArticulosBorrar = await responseArticulosBorrar.json() ;
      console.log("Borrado de articulos result")
      console.log(resultArticulosBorrar);
      ...
      for(const idx in ids){
        ...
        //console.log(auxId);
        let formData = new FormData();
            formData.append('idPedidoArticulo' , auxId.idPedidoArticulo);
            formData.append('idImagenes' , auxId.idImagenes);
        const response = await fetch('http://localhost/sieslite/api/pedidosImagenes.php?opc=2', {
              method: 'POST',
              body: formData
        });
        const result = await response.json() ;
      ...
      const response = await fetch('http://localhost/sieslite/api/pedidos.php?opc=3', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json'
          },
      ...
      Swal.fire({
        title: "Pedido modificado con exito",
