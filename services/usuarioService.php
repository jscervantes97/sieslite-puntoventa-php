...
class UsuarioService{
    ...
    public function searchByParams($params){
        ...
        foreach($data as $reg){
            ...
            $usuario->setModulos($reg[5]);
            $modulosString = $reg[5];
            $modulos = $this->usuarioRepository->findByQueryAssoc("select idModulo, nombre, url from modulos m where m.idModulo in ($modulosString);");
            $modulosArray = array();
            ...
            $usuario->setModulosArray($modulosArray);
    ...
    public function getAllModules(){
        ...
        $modulos = $this->usuarioRepository->findByQueryAssoc("select idModulo, nombre, url from modulos;");
        $modulosArray = array();
        foreach($modulos as $modulo){
            array_push($modulosArray , [
                'idModulo' => $modulo["idModulo"],
        ...
        return new ResultDTO("Data fetched" , $modulosArray);
    }
}
