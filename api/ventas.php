...
switch ($opcion){
    ...
    case 5 :
        $json = file_get_contents('php://input');
        $data = json_decode($json);
        $result = $ventaService->findAllVentas($data);
        http_response_code(200);
        echo json_encode($result->toJson()) ; 
        break ; 
    case 6 :
        $json = file_get_contents('php://input');
        $data = json_decode($json);
        $result = $ventaService->generarReporteVentasGastosIngresosDia($data);
        http_response_code(200); 
        echo json_encode($result->toJson()) ; 
        break ; 
    case 7 :
        $json = file_get_contents('php://input');
        $data = json_decode($json);
        $result = $ventaService->generarReporteVentasGastosIngresosDias($data);
        http_response_code(200); 
        echo json_encode($result->toJson()) ; 
        break ; 
