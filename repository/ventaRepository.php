...
class VentaRepository extends GenericRepository{
    ...
    public function findByVentaId($id){
        $searchVenta = null;  
        $search = $this->findByQuery("SELECT idVenta, v.status, totalArticulos, totalVenta, fechaVenta, u.nombreUsuario  as 'usuario', metodoPago, idCorte FROM ventas v inner join usuarios u on v.idUsuario = u.idUsuario where v.idVenta = $id ;");
        if(count($search) > 0){
    ...
    public function findVentaByQuery($query){
        return  $this->findByQueryAssoc($query);  
    }
}
?>